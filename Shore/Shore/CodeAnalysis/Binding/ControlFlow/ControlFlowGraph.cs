...
namespace Shore.CodeAnalysis.Binding.ControlFlow
{
    internal sealed class ControlFlowGraph
    {
        ...
        public sealed class BasicBlock
        {
            ...
            public override string ToString()
            {
                ...
                foreach (var statement in Statements) statement.WriteTo(writer);
        ...
        public sealed class GraphBuilder
        {
            ...
            private BasicBlock _start = new(true);
            private BasicBlock _end = new(false);
            ...
            public ControlFlowGraph Build(List<BasicBlock> blocks)
            {
                ...
                foreach (var block in blocks)
                {
                    foreach (var statement in block.Statements)
                    {
                        ...
                        if (statement is BoundLabelStatement labelStatement) _blockFromLabel.Add(labelStatement.BoundLabel, block);
                    }
                }
                    ...
                    foreach (var statement in current.Statements)
                    {
                        ...
                        switch (statement.Kind)
                        {
                            case BoundNodeKind.GotoStatement:
                                ...
                                var toBlock = _blockFromLabel[gs.BoundLabel];
                                Connect(current, toBlock);
                                break;
                            case BoundNodeKind.ConditionalGotoStatement:
                                var cgs = (BoundConditionalGotoStatement)statement;
                                var thenBlock = _blockFromLabel[cgs.BoundLabel];
                                var elseBlock = next;
        ...
        public static bool AllPathsReturn(BoundBlockStatement body)
        {
            ...
            foreach (var branch in graph.End.Incoming)
            {
                ...
                if (lastStatement == null || lastStatement.Kind != BoundNodeKind.ReturnStatement) return false;
            }
