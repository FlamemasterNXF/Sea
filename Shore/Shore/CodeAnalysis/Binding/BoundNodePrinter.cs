    ...
    internal static class BoundNodePrinter
    {
        ...
        public static void WriteTo(this BoundNode node, IndentedTextWriter writer)
        {
            switch (node.Kind)
            {
                ...
                case BoundNodeKind.NullExpression:
                    WriteErrorExpression((BoundNullExpression)node, writer);
                    break;
        ...
        private static void WriteNestedExpression(this IndentedTextWriter writer, int parentPrecedence, BoundExpression expression)
        {
            if (expression is BoundUnaryExpression unary)
                writer.WriteNestedExpression(parentPrecedence, unary.Op.TokType.GetUnaryOperatorPrecedence(), unary);
            else if (expression is BoundBinaryExpression binary)
                writer.WriteNestedExpression(parentPrecedence, binary.Op.TokType.GetBinaryOperatorPrecedence(), binary);
            else
        ...
        private static void WriteNestedExpression(this IndentedTextWriter writer, int parentPrecedence, int currentPrecedence, BoundExpression expression)
        {
            ...
            if (needsParenthesis) writer.WritePunctuation("(");
        ...
        private static void WriteBlockStatement(BoundBlockStatement node, IndentedTextWriter writer)
        {
            ...
            foreach (var s in node.Statements) s.WriteTo(writer);
        ...
        {
            writer.WriteKeyword(node.Variable.IsReadOnly ? $"readonly {node.Variable.Type} " : $"{node.Variable.Type} ");
        ...
        private static void WriteErrorExpression(BoundNullExpression node, IndentedTextWriter writer)
        {
        ...
        private static void WriteLiteralExpression(BoundLiteralExpression node, IndentedTextWriter writer)
        {
            ...
            if (node.Type == TypeSymbol.Bool) writer.WriteKeyword(value);
            else if (node.Type.ParentType == TypeSymbol.Number) writer.WriteNumber(value);
        ...
        {
            var op = SyntaxFacts.GetText(node.Op.TokType);
            var precedence = node.Op.TokType.GetUnaryOperatorPrecedence();
        ...
        {
            var op = SyntaxFacts.GetText(node.Op.TokType);
            var precedence = node.Op.TokType.GetBinaryOperatorPrecedence();
